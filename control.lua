...
function process_tick()
	...
	for i = #global.jammed_turrets, 1, -1 do -- Loop over table backwards because some entries get removed within the loop
		...
		if vehicle and vehicle.valid then
			if current_tick == lobal.jammed_turrets[i][3] - 84 then
				vehicle.surface.play_sound{path = "turret-repaired", position = vehicle.position}
			elseif not current_tick == lobal.jammed_turrets[i][3] then
				vehicle.relative_turret_orientation = lobal.jammed_turrets[i][2]
			else
...
function applyMalfunction(vehicle, type)
	if type == 1 then
		vehicle.surface.create_entity({name = "Get-Rekt-projectile", target = vehicle, force = game.forces.enemy, position = vehicle.position, speed = 1})
		global.jammed_turrets = global.jammed_turrets or {}
		global.jammed_turrets[vehicle.unit_number] = {vehicle, entity.relative_turret_orientation, game.tick + 5 * 60 + math.random(10 * 60)}
		vehicle.surface.create_entity{name = "tutorial-flying-text", position = vehicle.position, text = {"turret-jammed"}, color = {r = 1}}
		script.on_event(defines.events.on_tick, process_tick)
		return
	elseif type == 2 then
		vehicle.surface.create_entity({name = "Get-Rekt-slowdown-projectile", target = vehicle, force = game.forces.enemy, position = vehicle.position, speed = 1})
		return
	end
end
...
function makeRattleNoise(player, chance)
	if chance == 7 then
		player.play_sound{path = "Get-Rektd-rattle"}
	end
end
