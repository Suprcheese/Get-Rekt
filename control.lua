...
function process_tick()
	...
	for i = #global.jammed_turrets, 1, -1 do -- Loop over table backwards because some entries get removed within the loop
		...
		if vehicle and vehicle.valid then
			if current_tick == global.jammed_turrets[i][3] - 84 then -- Synchronize sound with actual unjamming time
			...
			elseif current_tick == global.jammed_turrets[i][3] then
				...
				vehicle.relative_turret_orientation = global.jammed_turrets[i][2] -- Can still fire the guns, just only in the one direction; can't turn the turret
			end
		else
			table.remove(global.jammed_turrets, i)
		end
...
function applyMalfunction(vehicle, type)
	if type == 1 then
		vehicle.surface.create_entity({name = "vehicle-damage-explosion", position = vehicle.position})
		global.jammed_turrets = global.jammed_turrets or {}
		local i = findEntity(vehicle)
		if i then
			global.jammed_turrets[i][3] = global.jammed_turrets[i][3] + 60 + math.random(5 * 60) -- Extend jammed duration a bit
			return
		else
			...
			script.on_event(defines.events.on_tick, process_tick)
			game.players[1].print("Turret jammed!")
			return
		end
	elseif type == 2 then
		game.players[1].print("Sticker!")
...
script.on_event(defines.events.on_player_changed_position, function(event)
	...
	if vehicle and vehicle.valid and vehicle.type == "car" then
		...
		if health_ratio >= 0.85 then -- (Near-)Full health, no penalties
			vehicle.effectivity_modifier = 1
			vehicle.friction_modifier = 1
			return
		elseif health_ratio < 0.85 then	-- Linearly scaling penalty based on health
			...
			return
		elseif health_ratio < 0.1 then -- Near-dead, maximum penalties applied with no further scaling based on health
